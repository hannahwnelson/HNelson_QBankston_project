...
# Convert copdval to numeric
outcomes_2020_sf$copdval <- as.numeric(outcomes_2020_sf$copdval)

# Create a leaflet map with gradient colors
copd_map <- leaflet(outcomes_2020_sf) %>%
  addProviderTiles("CartoDB.Positron") %>%  # You can choose a different tile provider
  addPolygons(
    fillColor = ~cut(copdval, quantile(outcomes_2020_sf$copdval, na.rm = TRUE), include.lowest = TRUE),
    fillOpacity = 0.7,
    color = "white",
    weight = 1,
    label = ~as.character(outcomes_2020_sf$copdval),
  ...
  addLegend(
    ...
    pal = colorFactor(
      palette = "Blues",
      domain = cut(outcomes_2020_sf$copdval, quantile(outcomes_2020_sf$copdval, na.rm = TRUE), include.lowest = TRUE)
    ),
    values = ~outcomes_2020_sf$copdval,
    title = "COPD Values",
    opacity = 0.7
  )
